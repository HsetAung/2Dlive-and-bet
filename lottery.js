$(document).ready(function () {
  ...
  yourMoney.value = new Intl.NumberFormat().format(totalSum.toFixed(0)) + "Ks";
  ...
  /**
   * checking  already a selected item or not
   * @param {*} digit
   * @returns true/false
   */
   ...
   */
  function updateCart() {
    let grandTotal = 0; // cart of initial total value
    ...
    selectItem_Array.forEach((item, index) => {
      ...
      const bin_Img = $('<img src="./bin2.png" alt="" class="bimImg">');
      removeFromCart();
      /**
      ...
      function calculate_totalVal_forCart() {
        ...
        $("#grand").text(
          new Intl.NumberFormat().format(grandTotal.toFixed(0)) + "Ks"
        );
      }
  ...
  function updateYourMoney() {
    ...
    yourMoney.value =
      ...
      console.log("totalVal",yourMoney.value)
  }
